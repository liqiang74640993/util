 ...
 *                  在总存储顺序（TSO）场景中，省略显式屏障代码的生成，以避免出现不必要的指令。
...
public:
     ...
     *  如果消费者在同一时间尝试读取数据，该函数可能会在未执行任何操作的情况下返回。
     */
     ...
     * head 通常表示生产者写入数据的位置，tail 表示消费者读取数据的位置。
     * 当 tail 追上 head 时，意味着缓冲区中的数据都已被消费，缓冲区为空。
     * 通过将 head 的值赋给 tail，相当于让消费者 “追上” 生产者，
     * 使得缓冲区看起来像是被清空了，后续生产者可以继续写入新的数据。
     */
    void consumerClear(void) {
        //原子地将head的值写入tail，不保证该写入操作与其他操作的顺序
        //only atomic set the tail value to the head
...
private:
    ...
    //平凡类型是指满足特定条件的类型，这些条件包括具有平凡的默认构造函数、拷贝构造函数、移动构造函数、拷贝赋值运算符、移动赋值运算符和析构函数，并且没有虚函数或虚基类。
    //这句话是一个 C++ 的 static_assert 语句，其主要目的是在编译阶段进行条件检查，确保类型 T 是平凡类型（trivial type），
    //如果不满足这个条件，就会导致编译失败并输出指定的错误信息。
    //为了性能考虑（对象仅仅使用赋值或者memcpy就可以完成拷贝），无锁队列为了实现高性能，如果对象具有复杂的构造，拷贝等，就不适合使用该环形缓冲区
