...
*/
uint8_t rb_init(ring_buffer *rb_handle, uint8_t *buffer_addr ,size_t buffer_size)
{
    //缓冲区数组空间必须大于2且小于数据类型最大值
    if(buffer_size < 2 || buffer_size == (size_t)-1)//-1转换为无符号数就是无符号数的最大值
...
*/
uint8_t rb_delete(ring_buffer *rb_handle, size_t Length)
{
...
uint8_t rb_write_byte(ring_buffer *rb_handle, uint8_t data)
{
    ...
    {
        //写数据时，是先操作rear指针，然后将rear指针+1，rear指针的位置不存放数据
 ...
 *      \arg RING_BUFFER_ERROR: 写入失败
*/
uint8_t rb_write_string(ring_buffer *rb_handle, uint8_t *input_addr, size_t write_Length)
{
    //如果不够存储空间存放新数据,返回错误
 ...
 *      \arg RING_BUFFER_ERROR: 读取失败
*/
uint8_t rb_read_string(ring_buffer *rb_handle, uint8_t *output_addr, size_t read_Length)
{
...
*/
size_t rb_get_length(ring_buffer *rb_handle)
{
 ...
 * \return 返回缓冲区可用储存空间
*/
size_t rb_get_freeSize(ring_buffer *rb_handle)
{
